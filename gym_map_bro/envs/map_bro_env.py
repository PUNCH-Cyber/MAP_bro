...
class broEnv(gym.Env):
	...
	def _take_action(self, action, value):
		reward = 0
		#print(f'start of action {values0}')
		if(action == 0):
	...
	def decay_step(self):
		self.values0_init[:,1] = self.values0_init[:,1]*0.9
	...
	def render(self, mode='human', close=True):
		time = self.values0_init[:,0]
		value = self.inv_decay(self.values0_init[:,1],self.values0_init[:,0])
